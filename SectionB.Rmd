...
Q_{X}(p)=F_{X}^{-1}(x)
\]
This can be derived by rearranging cumulative distribution function function
\[p=1-e^{-\lambda x}\]
\[e^{-\lambda x}=1-p\]
\[-\lambda x=\ln (1-p)\]
...
\[\hat{\lambda_{n}}=\operatorname*{argmin}_\lambda l(\lambda;x_{1},...,x_{n})\]
...
Hence, the maximum likelihood estimate  $ \hat{\lambda}_{MLE} $ for $\lambda_{0}$ is$\frac{n}{\sum_{i=1}^{n}x_{i}}$
